...
interface TempStorage {
  _tempSettings?: MagicWordsSettings;
  _tempCallbacks?: MagicWordsSettingsCallbacks;
}
...
export class MagicWordsSettingsPanel extends GameSettingsPanel {
  ...
  constructor(
    ...
    const temp = MagicWordsSettingsPanel as unknown as TempStorage;
  ...
  private buildControlsForState(
    ...
    if (deviceState === 'phonePortrait') {
      ...
      dialogCell.y = 0;
      avatarCell.x = colX;
      avatarCell.y = dialogCell.y + sliderRowHeight + rowGapY;
      yOffsetCell.x = colX;
      yOffsetCell.y = avatarCell.y + sliderRowHeight + rowGapY;
      ...
      fakeLagCell.x = row2StartX;
      fakeLagCell.y = row2Y;
      keepCell.x = row2StartX + sliderWidth + colGapX;
      keepCell.y = row2Y;
    }
  ...
  public override scaleToFit(): void {
    ...
    const minPadding = useCompactPadding
      ? SCENE_LAYOUT.screenPaddingPhone
      : SCENE_LAYOUT.screenPadding;
