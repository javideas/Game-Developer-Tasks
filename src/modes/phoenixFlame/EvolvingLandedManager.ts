...
export class EvolvingLandedManager {
    ...
    if (evolvingSprite.clicks >= this.config.clicksPerLevel) {
      ...
      if (evolvingSprite.level >= 3) {
        ...
        if (this.config.onEggCreated) {
          this.config.onEggCreated(
            evolvingSprite.sprite.x,
            evolvingSprite.sprite.y,
            evolvingSprite.sprite
          );
        }
        return;
      }
  ...
  private getTexturesForLevel(level: number): Texture[] {
    switch (level) {
      case 0:
        return this.level0Textures;
      case 1:
        return this.level1Textures;
      case 2:
        return this.level2Textures;
      case 3:
        return this.level3Textures;
      default:
        return this.level3Textures;
    }
  }
  ...
  private restartShrinkTimer(evolvingSprite: EvolvingSprite): void {
    ...
    evolvingSprite.shrinkTween = gsap
      .timeline()
   ...
   */
  public spawnLanded(x: number, y: number, scale: number): boolean {
