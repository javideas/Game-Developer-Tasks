...
export class AceOfShadowsModeCreative implements GameMode {
   ...
   */
  private initialDealtRanks: Set<number> = new Set();
   ...
   */
  private revealedRanksThisBatch: Set<number> = new Set();
  ...
  private buildTableau(): void {
    ...
    const { rows, cardsPerRow, rotatedRows, rotationAngle, fanRotationRows, rowCardOrientations } = this.currentLayout;
      ...
      for (let col = 0; col < cardsInRow; col++) {
        ...
        if (rotatedRows?.includes(row)) {
          ...
          if (angle !== 0) {
            ...
            if (fanRotationRows?.includes(row)) {
    ...
    for (let row = 0; row < rows - 1; row++) {
      ...
      for (const blockedCard of blockedCards) {
        ...
        for (let bCol = 0; bCol < blockerCount; bCol++) {
          ...
          if (Math.abs(blockerX - blockedX) <= overlapThreshold) blockedCard.blockedBy.push(blocker);
        }
      }
    }
  }
  ...
  private logGameState(event: string): void {
    ...
    if (this.wasteCard) {
      ...
      console.log(`  → Can play: ${this.rankToString(validLow)} or ${this.rankToString(validHigh)}`);
    } else {
    ...
    for (let row = 0; row < this.tableauCards.length; row++) {
      ...
      for (const card of rowCards) {
        ...
        if (!card.isFaceUp) {
          ...
          const validMove = this.wasteCard ? this.isValidMove(card.rank, this.wasteCard.rank) : true;
  ...
  private buildStockPile(): void {
    ...
    this.stockCountText = new Text(String(stockCount), new TextStyle({
  ...
  private showWinMessage(): void {
    ...
    // Win message
    const winText = new Text('🎉 Game Won! 🎉', new TextStyle({
      fontFamily: 'Arial, sans-serif',
      fontSize: 48,
      fontWeight: 'bold',
      fill: '#FFD700',
    ...
    // Restart countdown
    const countdownText = new Text('Restarting in 3...', new TextStyle({
      fontFamily: 'Arial, sans-serif',
      fontSize: 24,
      fill: '#FFFFFF',
    }));
  ...
  private buildLayoutSelector(): void {
    ...
    const label = new Text(this.currentLayout.name, new TextStyle({
      fontFamily: 'Arial, sans-serif',
      fontSize: 14,
      fill: '#ffffff',
    }));
    ...
    // Dropdown caret
    const caret = new Text('▾', new TextStyle({
      fontFamily: 'Arial, sans-serif',
      fontSize: 14,
      fill: '#bbbbbb',
    }));
    ...
    for (const opt of options) {
      const optText = new Text(TABLEAU_LAYOUTS[opt.type].name, new TextStyle({
        fontFamily: 'Arial, sans-serif',
        fontSize: 14,
        fill: '#ffffff',
      }));
      ...
      optText.on('pointerover', () => { optText.style.fill = '#ffd27a'; });
      optText.on('pointerout', () => { optText.style.fill = '#ffffff'; });
