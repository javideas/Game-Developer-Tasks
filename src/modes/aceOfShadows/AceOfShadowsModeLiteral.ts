...
export class AceOfShadowsModeLiteral implements GameMode {
  ...
  /** Reset version counter - incremented on each deck reset to invalidate in-flight animations */
  private resetVersion = 0;

  /** Frame ID for reset - blocks shadow updates on the same frame as reset */
  private resetFrameId = 0;
  ...
  stop(): void {
    ...
    // Kill tweens on ALL cards in both stacks explicitly
    // This catches any in-flight animations that might have been started
    for (const card of this.leftStack) {
      gsap.killTweensOf(card);
      gsap.killTweensOf(card.scale);
      gsap.killTweensOf(card.position);
    }
    for (const card of this.rightStack) {
      gsap.killTweensOf(card);
      gsap.killTweensOf(card.scale);
      gsap.killTweensOf(card.position);
    }
    ...
    if (this.shadowLayer) {
      killTweensRecursive(this.shadowLayer);
    }
    if (this.stackShadowLayer) {
      killTweensRecursive(this.stackShadowLayer);
    }
  ...
  private createCardWithShadow(cardTexture: Texture, textureName: string): CardContainer {
    // Use proper CardContainer class instead of type assertion
    const cardContainer = new CardContainer(cardTexture, textureName);
  ...
  private updateDualShadows(cardGameX: number, _cardGameY: number, cardScaleX?: number): void {
    // Block shadow updates during reset frame to prevent stale shadow appearing
    if (this.resetFrameId !== 0) return;
  ...
  private moveTopCardToRight(): void {
    ...
    // Capture reset version to invalidate callbacks if reset occurs during animation
    const animationResetVersion = this.resetVersion;
    ...
    if (this.animationMode === 'spiral' && cardSprite) {
      ...
      const backTexture = this.getCardBackTexture(cardContainer.textureName);
      ...
      const tl = gsap.timeline({
        onUpdate: () => {
          ...
          motionBlur.velocity = new Point(
            velX * blurScale * containerScale,
            velY * blurScale * containerScale
          );
        ...
        onComplete: () => {
          // Guard: Don't complete if disposed or reset occurred during animation
          if (this.isDisposed || this.resetVersion !== animationResetVersion) return;
          ...
          cardContainer.isShowingFace = false;
          if (backTexture && !hasFlipped) {
            cardSprite.texture = backTexture;
          }
    ...
    if (this.animationMode === 'spiral' && cardSprite) {
      ...
      const tl = gsap.timeline({
        onUpdate: () => {
          ...
          motionBlur.velocity = new Point(
            velX2 * blurScale2 * containerScale2,
            velY2 * blurScale2 * containerScale2
          );
          ...
          if (!flipStarted && isClear) {
            ...
            gsap.to(cardContainer.scale, {
              ...
              onComplete: () => {
                if (this.isDisposed) return;
                if (!hasFlipped) {
                  cardSprite.texture = cardContainer.faceTexture;
                  hasFlipped = true;
                }
                gsap.to(cardContainer.scale, {
                  x: CARD_CONFIG.scale,
  ...
  private createSettingsPanel(): void {
    ...
    this.settingsPanel = new LiteralModeSettingsPanel(
      ...
      initialSettings,
      {
        ...
        onResetDeck: target => {
          console.log('[DeckToggle] MODE onResetDeck callback:', {
            target,
            modeActiveDeckBefore: self.activeDeck,
            settingsPanelExists: !!self.settingsPanel,
          });
  ...
  private resetAllCardsTo(target: 'left' | 'right'): void {
    // Block shadow updates for this frame to prevent stale shadow appearing
    if (this.resetFrameId) {
      cancelAnimationFrame(this.resetFrameId);
    }
    this.resetFrameId = requestAnimationFrame(() => {
      this.resetFrameId = 0; // Clear after one frame
    });

    // Increment reset version to invalidate any in-flight animation callbacks
    this.resetVersion++;

    // Stop animation interval first
    if (this.moveIntervalId) {
      clearInterval(this.moveIntervalId);
      this.moveIntervalId = null;
    }

    // Kill ALL animations recursively on all containers (complete cleanup)
    killTweensRecursive(this.leftContainer);
    killTweensRecursive(this.rightContainer);
    if (this.movingCardLayer) {
      killTweensRecursive(this.movingCardLayer);
    ...
    // Clear ALL layers completely (including shadow sprites - they'll be recreated)
    if (this.movingCardLayer) {
    ...
    if (this.stackShadowLayer) {
      this.stackShadowLayer.removeChildren();
    }
    this.leftContainer.removeChildren();
    this.rightContainer.removeChildren();

    // Clear shadow references (they were destroyed with removeChildren)
    this.floorShadow = null;
    this.stackShadow = null;
    this.stackShadowMask = null;
    ...
    // Reset shadow state
    this.stackShadowInfo = { visible: false, x: 0, y: 0 };
    ...
    // Recreate shadow sprites (they were cleared above)
    this.createShadowSprites();
    ...
    for (let i = 0; i < CARD_CONFIG.totalCards; i++) {
      ...
      // Deck A (left): Cards show FACE (default from createCardWithShadow)
      // Deck B (right): Cards show BACK
      if (target === 'right') {
        const cardSprite = this.getCardSprite(cardContainer);
